    ...
    for row in range(len(matrix)):
        ...
        for col in range(len(matrix[0])):
            ...
            if is_valid_char(matrix[row][col]) and (col - 1 < len(matrix[0]) and not is_valid_char(matrix[row][col - 1])):
                ...
                # subset of lines
                subset = matrix[row - 1 if row > 0 else 0 : row + 2 if row < len(matrix) - 1 else row + 1]
                # subset of columns
                for rowSubset in range(len(subset)):
                    subset[rowSubset] = subset[rowSubset][col - 1 if col > 0 else 0 : col + numbersize + 1 if col + numbersize < len(matrix[0]) else col + numbersize]

                # get all chars in the subset, deduplicated
                charset = set(join_rows(subset));
                # remove all numbers and . from the charset
                charset = list(filter(lambda x: not x.isdigit() and not x == '.', charset))
